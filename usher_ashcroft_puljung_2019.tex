...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% PREAMBLE
\documentclass[10pt,lineno, doublespacing]{elife_modified}
...
Cells were plated on 19 mm coverslips coated with poly-\sc{l}-lysine and transfected as described above.
...
\bibliography{usher_ashcroft_puljung_2019}
...
\cmidrule{2-5}
                 ...
                 &                        & $I_{max}$ & 1.00  & 84.35\\
\cmidrule{2-5}
                 & Kir6.2*-GFP+SUR-K205A  & $IC_{50}$ & -3.78 & 0.45\\
                 & n = 9                  & $h$       & 0.75  & 0.30\\
                 &                        & $I_{max}$ & 1.00  & 0.29\\
\cmidrule{2-5}
                 & Kir6.2*-GFP+SUR-K205E  & $IC_{50}$ & -3.20 & 2.15\\
                 & n = 9                  & $h$       & 0.79  & 0.84\\
                 &                        & $I_{max}$ & 1.00  & 1.77\\
\cmidrule{2-5}\global\rownum=1\relax
\textbf{Fluorescence Quenching} & & & & \\
\midrule
\textit{TNP-ATP} & Kir6.2*-GFP+SUR1       & $EC_{50}$ & -4.11 & 0.09\\
                 & n = 9                  & $h$       & 0.87  & 0.11\\
                 &                        & $E_{max}$ & 1.00  & 0.06\\
\cmidrule{2-5}
                 & Kir6.2*-C166S-GFP+SUR1 & $EC_{50}$ & -4.17 & 0.23\\
                 & n = 6                  & $h$       & 0.84  & 0.27\\
                 &                        & $E_{max}$ & 1.00  & 0.14\\
\cmidrule{2-5}
                 & Kir6.2*-GFP+SUR-K205A  & $EC_{50}$ & -3.69 & 0.42\\
                 & n = 9                  & $h$       & 0.73  & 0.25\\
                 &                        & $E_{max}$ & 1.00  & 0.27\\
\cmidrule{2-5}
                 & Kir6.2*-GFP+SUR-K205E  & $EC_{50}$ & -3.37 & 1.10\\
                 & n = 9                  & $h$       & 0.74  & 0.47\\
                 &                        & $E_{max}$ & 1.00  & 0.79\\
\midrule
\end{tabular}
\caption{\label{table:pc} \textbf{Table 2: Hill fit parameters from excised patches.} $EC_{50}$ values and their standard errors are reported in $\log_{10}M^{-1}$.}
\endgroup{}
\end{table}

\begin{table}
\rowcolors{3}{white}{light-grey}
\centering\begingroup
...
n = 9                  & $D$     & 0.62  & 0.42  & 0.74 \\
                       & $K_{A}$ & 3.40  & 3.21  & 3.62 \global\rownum=0\relax\\
\cmidrule{2-5}
\textbf{Single-site} & & & & \\
\midrule
...
n = 9                  & $D$     & 0.30  & 0.13  & 0.47 \\
                       & $K_{A}$ & 3.31  & 3.13  & 3.50 \global\rownum=0\relax\\
\cmidrule{2-5}
